...
The label is added **manually** by the PR author when they consider the PR ready for review.

### Reviewer Pool

| GitHub Handle | Name |
|---|---|
| `souvikghosh04` | Souvik |
| `Aniruddh25` | Aniruddh |
| `aaronburtle` | Aaron |
| `anushakolan` | Anusha |
| `RubenCerna2079` | Ruben |
...
Size labels are **manual and optional**. If no size label is set, the PR gets a default weight of 1 and the bot still assigns reviewers — it just won't do weighted load balancing.

### Reviewer Lifecycle & Availability

A reviewer goes through a defined lifecycle per PR assignment. The bot respects this when deciding who is available.
...
While a reviewer is in the active-review state for a PR, that PR counts toward their load. The bot **prefers unblocked reviewers** (those with no pending active reviews) when selecting candidates.

#### Round-Robin Fallback

When **every reviewer** in the pool has active assignments, the bot continues assigning using the existing weighted load-balancing logic:
...
If a PR author **requests a re-review**, the assignment lifecycle **starts from scratch**:
...
To determine active vs. freed reviewers, the bot calls `pulls.listReviews` (paginated) for each eligible PR:
...
The bot starts in **dry-run mode** (`DRY_RUN = true`). In this mode it logs `[DRY RUN] Would assign [...]` but does not actually add reviewers.
...
> Jerry (`JerryNixon`) has been removed from the pool ΓÇö PM role, should not review code PRs.
...
Size labels are **manual and optional**. If no size label is set, the PR gets a default weight of 1 and the bot still assigns reviewers ΓÇö it just won't do weighted load balancing. Auto-detection of PR size is a possible future enhancement.
...
A reviewer is considered **actively reviewing** a PR ΓÇö and therefore **blocked** ΓÇö from the moment they are assigned until they complete one of the following actions:
- **Approve** the PR
- **Request changes** on the PR
- **Comment** (submit a review with comments)
- **The PR is closed or merged**
...
When **every reviewer** in the pool has active assignments, the bot does not stop ΓÇö it continues assigning using the existing weighted load-balancing logic:
- Candidates are sorted by current weighted load (ascending).
- The reviewer(s) with the **fewest active assignments** are picked first.
- This naturally round-robins through the pool as load accumulates.
...
- The bot treats the PR as needing reviewers again.
- The same rules apply ΓÇö load balancing, author exclusion, active-review checks.
...
- **Active reviewer:** Assigned to the PR but has **not** submitted a review yet ΓåÆ counts toward load.
- **Freed reviewer:** Has submitted a review (approve, request changes, or comment) ΓåÆ does **not** count toward load.
...
| `REQUIRED_REVIEWERS` | Script constant | `2` | Number of reviewers per PR |
| `STALE_DAYS` | Script constant | `90` | PRs not updated in this many days are skipped |
| `DRY_RUN` | Script constant / workflow input | `true` | When true, logs only ΓÇö no actual assignments |
| `assign-for-review` | PR label | ΓÇö | Label that opts a PR into auto-assignment (removed after assignment when live) |
| Size labels | PR labels | `size-medium`, `size-large` | Increase PR weight for load balancing |
| `priority-high` | PR label | ΓÇö | Adds +1 to PR weight |
...
3. Find the latest **"PR Review Assignment Bot"** workflow run.
4. Open the **"Assign reviewers to eligible PRs"** step log.
5. Look for `[DRY RUN] Would assign [...]` lines ΓÇö these show who would be assigned and to which PR.
6. Verify the assignments look correct (author excluded, load balanced, right reviewer count).

You can also trigger a manual run from the **Actions** tab ΓåÆ **PR Review Assignment Bot** ΓåÆ **Run workflow** and choose dry-run `true` or `false`.
...
| April 8 | **BUG FIX:** CODEOWNERS inflated `requested_reviewers` ΓåÆ switched to `pr.assignees` |
| April 8 | **BUG FIX:** `pulls.requestReviewers` wrote to Reviewers field ΓåÆ switched to `issues.addAssignees` |
| April 8 | **BUG FIX:** Dry-run toggle `\|\|` expression always true ΓåÆ fixed with `context.eventName` check |
...
| # | Item | Owner | Status |
|---|---|---|---|
| 1 | **Decide on Aniruddh's inclusion** ΓÇö remove to reduce his load, or keep as equal | Team | Pending |
...
3. **Check logs** ΓÇö verify `[DRY RUN] Would assign [...]` output:
...
4. **Flip to live** ΓÇö set `DRY_RUN` to `false`.
...
>
> **How it works:**
> - Scans all open PRs for the `assign-for-review` label ΓÇö this is the opt-in mechanism.
...
> **Subject: PR Review Assignment Bot ΓÇö Ready for Testing**
>
> Hi Team,
>
...
> - Uses **weighted load balancing** ΓÇö reviewers with the lightest active load are picked first.
> - **Prefers available reviewers** ΓÇö those not currently reviewing any PR are prioritized. If everyone is busy, it round-robins by least load.
...
>
> **What changed:**
> - Fixed CODEOWNERS bug ΓÇö assignments now use the **Assignees** field (not Reviewers), so CODEOWNERS no longer interferes.
> - Fixed API ΓÇö bot now uses `issues.addAssignees` to write to the correct field.
