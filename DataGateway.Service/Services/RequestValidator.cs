...
namespace Azure.DataGateway.Service.Services
{
    ...
    public class RequestValidator
    {
        ...
        IMetadataStoreProvider configurationProvider)
        {
            ...
            {
                if (ValidateColumn(column, fieldsInRequestBody))
                {
        ...
        /// and vice versa.
        /// </summary>
        /// <param name="upsertRequestCtx">Update Request context containing the request body.</param>
        /// <param name="configurationProvider">Configuration provider that enables referencing DB schema in config.</param>
        /// <exception cref="DataGatewayException"></exception>
        public static void ValidateUpdateRequestContext(
        UpsertRequestContext upsertRequestCtx,
        IMetadataStoreProvider configurationProvider)
        {
            IEnumerable<string> fieldsInRequestBody = upsertRequestCtx.FieldValuePairsInBody.Keys;
            ...
            foreach (KeyValuePair<string, ColumnDefinition> column in tableDefinition.Columns)
            {
                ...
                // AutoGenerated fields in the request body are not allowed.
                if (column.Value.IsAutoGenerated && fieldsInRequestBody.Contains(column.Key))
                {
                    throw new DataGatewayException(
                        message: "Invalid request body. Either insufficient or unnecessary field(s) supplied.",
                        statusCode: HttpStatusCode.BadRequest,
                        subStatusCode: DataGatewayException.SubStatusCodes.BadRequest);
                }
                ...
                if (!column.Value.IsNullable && fieldsInRequestBody.Contains(column.Key))
                {
                    ...
                    {
                        throw new DataGatewayException(
                            message: "Invalid request body. Either insufficient or unnecessary field value(s) supplied.",
                            statusCode: HttpStatusCode.BadRequest,
                            subStatusCode: DataGatewayException.SubStatusCodes.BadRequest);
                    }
                }
        ...
        private static bool ValidateColumn(KeyValuePair<string, ColumnDefinition> column, IEnumerable<string> fieldsInRequestBody)
        {
            string message;
            // Autogenerated values should not be be in the body.
            if (column.Value.IsAutoGenerated && fieldsInRequestBody.Contains(column.Key))
            {
                message = $"Invalid request body. Field not allowed in body: {column.Key}.";
            }
            // Non-Autogenerated values must be in the body unless nullable.
            else if (!column.Value.IsAutoGenerated && !column.Value.IsNullable && !fieldsInRequestBody.Contains(column.Key))
            {
