    ...
    public class RequestValidator
    {
            ...
            ISqlMetadataProvider sqlMetadataProvider)
        {
            ...
            foreach (KeyValuePair<string, ColumnDefinition> column in tableDefinition.Columns)
            {
                ...
                {
                    if (insertRequestCtx.FieldValuePairsInBody[exposedName!] == null)
                    {
                        throw new DataGatewayException(
                ...
                if (ValidateColumn(column.Value, exposedName!, fieldsInRequestBody, isReplacementUpdate: true))
                {
            ...
            ISqlMetadataProvider sqlMetadataProvider)
        {
            ...
            foreach (KeyValuePair<string, ColumnDefinition> column in tableDefinition.Columns)
            {
                sqlMetadataProvider.TryGetExposedColumnName(entityName: upsertRequestCtx.EntityName,
                                                            backingFieldName: column.Key,
                                                            out string? exposedName);
                ...
                if (!column.Value.IsNullable && fieldsInRequestBody.Contains(exposedName))
                {
                    if (upsertRequestCtx.FieldValuePairsInBody[exposedName!] == null)
                    {
                        throw new DataGatewayException(
                            message: $"Invalid value for field {exposedName} in request body.",
                            statusCode: HttpStatusCode.BadRequest,
        ...
        private static bool ValidateColumn(ColumnDefinition column, string exposedName, IEnumerable<string> fieldsInRequestBody, bool isReplacementUpdate)
        {
            string message;
            ...
            else if (!column.IsAutoGenerated && !column.IsNullable && !column.HasDefault && !fieldsInRequestBody.Contains(exposedName) && isReplacementUpdate)
            {
