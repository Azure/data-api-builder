...
namespace Azure.DataGateway.Service.Services
{
    ...
    public class RequestValidator
    {
        ...
        public static void ValidateUdateRequestContext(UpdateRequestContext updateRequestContext, IMetadataStoreProvider metadataStoreProvider)
        {
            IEnumerable<string> fieldsInRequestBody = updateRequestContext.FieldValuePairsInBody.Keys;
            TableDefinition tableDefinition =
                TryGetTableDefinition(updateRequestContext.EntityName, metadataStoreProvider);
            ...
            foreach (KeyValuePair<string, ColumnDefinition> column in tableDefinition.Columns)
            {
                ...
                bool isReplacementUpdate = (updateRequestContext.OperationType == Operation.Upsert) ? true : false;
                ...
                if (!isReplacementUpdate)
                {
                    if (!column.Value.IsNullable && fieldsInRequestBody.Contains(column.Key))
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(updateRequestContext.FieldValuePairsInBody[column.Key].ToString()))
                        {
                            throw new DataGatewayException(
        ...
        /// <summary>
        /// Validates the primarykeyroute is populated and returns the requestBody
        /// as a JsonElement.
        /// </summary>
        /// <param name="primaryKeyRoute">The url's primary key route.</param>
        /// <param name="requestBody">The JSON body from the request.</param>
        /// <returns>JSON Element representing the request body.</returns>
        internal static JsonElement ValidateUpdateRequest(string? primaryKeyRoute, string requestBody)
        {
            ...
            JsonElement updatePayloadRoot = new();

            if (!string.IsNullOrEmpty(requestBody))
            {
                updatePayloadRoot = GetInsertPayload(requestBody);
            }

            return updatePayloadRoot;
        }
