...
2. **McpStdioServer** (`src/Azure.DataApiBuilder.Mcp/Core/McpStdioServer.cs`)
   ...
   - Enables/disables MCP log notifications
...
3. **Program.cs** (`src/Service/Program.cs`)
   ...
   - Registers McpLoggerProvider for MCP notifications

4. **MCP Notification Logging** (`src/Azure.DataApiBuilder.Mcp/Telemetry/`)
   - `McpLogNotificationWriter`: Sends logs as MCP `notifications/message` JSON-RPC notifications
   - `McpLogger`: ILogger implementation that sends logs via the notification writer
   - `McpLoggerProvider`: ILoggerProvider that creates McpLogger instances

## MCP Log Notifications

When running in MCP stdio mode and a client calls `logging/setLevel` with a level other than "none",
DAB sends log messages as MCP `notifications/message` JSON-RPC notifications to stdout.
...
```json
{
  ...
  "params": {
    "level": "info",
    "logger": "Azure.DataApiBuilder.Service.SomeClass",
    "data": "The log message"
  }
}
```

### Supported MCP Log Levels
...
1. MCP client sends `logging/setLevel` with desired level (e.g., "info")
2. DAB enables the `McpLogNotificationWriter`
3. All log messages at or above the requested level are sent as `notifications/message` to stdout
4. MCP clients (like MCP Inspector) display these in their log panel
