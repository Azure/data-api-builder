...
Once the container is created, we can import the sample data from `books.json` which is placed in the ['azure-cosmos-db'](../../samples/getting-started/azure-cosmos-db/books.json) to the book collection by using the add new item option (Make sure you add one by one item) in the Azure Data Explorer.
