...
3. Execute the command `dotnet build -p:generateConfigFiles=<database_type>` in the directory `data-api-builder\src` to build the project and generate the config file that can be used when starting DAB. The config file will be generated in the directory `data-api-builder\src\Service`. `MsSql`, `PostgreSql`,`Cosmos` and `MySql` are the values that can be used with `generateConfigFiles`. Only the config file for the specified database type will be generated.
