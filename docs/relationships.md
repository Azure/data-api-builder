...
The option `relationship.fields` allows you to define which fields will be used from the entity being updated (`Series`), and which fields will be used from the target entity (`Book`), to connect the data from one entity to the other.
...
To allow such scenario to be gracefully handled by DAB, all is needed is to create the related entities and mappings in the configuration file. Assuming the `Book` and `Author` entity are already in the configuration file (see the Getting Started guide if you need help on that):
