...
namespace Azure.DataApiBuilder.Mcp.Utils
{
    ...
    {
        public static CallToolResult PermissionDenied(string entityName, string operation, string detail, ILogger? logger)
        {
            string message = $"Permission denied for {operation} on entity '{entityName}'. {detail}";
            return McpResponseBuilder.BuildErrorResult(Model.McpErrorCode.PermissionDenied.ToString(), message, logger);
        }
        ...
        public static CallToolResult ToolDisabled(string toolName, ILogger? logger)
        {
            ...
            return McpResponseBuilder.BuildErrorResult(Model.McpErrorCode.ToolDisabled.ToString(), message, logger);
        }
    }
}
