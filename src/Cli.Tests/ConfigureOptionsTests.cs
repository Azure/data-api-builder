...
namespace Cli.Tests
{
    ...
    public class ConfigureOptionsTests : VerifyBase
    {
        ...
        /// <summary>
        /// Tests that running the "configure --file" commands on a config without file properties results
        /// in a valid config being generated.
        /// </summary>
        [TestMethod]
        public void TestAddFileOptions()
        {
            // Arrange
            _fileSystem!.AddFile(TEST_RUNTIME_CONFIG_FILE, new MockFileData(INITIAL_CONFIG));
            ...
            ConfigureOptions options = new(
                fileEnabled: CliBool.True,
                filePath: "/custom/log/path.txt",
                fileRollingInterval: "Hour",
                fileRetainedFileCountLimit: 5,
                fileFileSizeLimitBytes: 2097152,
                config: TEST_RUNTIME_CONFIG_FILE
            );
