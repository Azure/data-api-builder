...
namespace Azure.DataApiBuilder.Config
{
        ...
        Dictionary<string, string>? Mappings)
    {
        ...
        /// (JSON Object) Explicit Stored Procedure and Function operation type "query" or "mutation"
        /// </summary>
        ...
        public bool TryProcessGraphQLNamingConfig()
        {
            ...
            if (GraphQL is JsonElement configElement)
            {
                ...
                else if (configElement.ValueKind is JsonValueKind.Object)
                {
                    ...
                    // Only stored procedure and function configuration can override the GraphQL operation type.
                    // When the entity is a stored procedure or function, GraphQL metadata will either be:
                    // - GraphQLDatabaseExecutableEntityOperationSettings when only operation is configured.
                    // - GraphQLDatabaseExecutableEntityVerboseSettings when both type and operation are configured.
                    // This verbosity is necessary to ensure the operation key/value pair is not persisted in the runtime config
                    // for non stored procedure/function entity types.
                    if (ObjectType.IsDatabaseExecutableType())
                    {
                        ...
                        {
                            GraphQL = new GraphQLDatabaseExecutableEntityOperationSettings(GraphQLOperation: graphQLOperation.ToString());
                        }
                        else
                        {
                            GraphQL = new GraphQLDatabaseExecutableEntityVerboseSettings(Type: typeConfiguration, GraphQLOperation: graphQLOperation.ToString());
                        }
                    }
                    else
                    {
        ...
        /// <summary>
        /// Returns the GraphQL operation that is configured for the stored procedure or function as a string.
        /// </summary>
        ...
        public GraphQLOperation? FetchConfiguredGraphQLOperation()
        {
            ...
            else if (GraphQL is GraphQLDatabaseExecutableEntityOperationSettings operationSettings)
            {
                return Enum.TryParse(operationSettings.GraphQLOperation, ignoreCase: true, out GraphQLOperation operation) ? operation : null;
            }
            else if (GraphQL is GraphQLDatabaseExecutableEntityVerboseSettings verboseSettings)
            {
        ...
        /// <summary>
        /// Gets the REST HTTP methods configured for the stored procedure or function
        /// </summary>
        /// <returns>An array of HTTP methods configured</returns>
        public RestMethod[]? GetRestMethodsConfiguredForDatabaseExecutable()
        {
            if (Rest is not null && ((JsonElement)Rest).ValueKind is JsonValueKind.Object)
            {
                ...
                {
                    RestDatabaseExecutableEntitySettings? restSpSettings = JsonSerializer.Deserialize<RestDatabaseExecutableEntitySettings>((JsonElement)Rest, RuntimeConfig.SerializerOptions);
                ...
                {
                    RestDatabaseExecutableEntityVerboseSettings? restSpSettings = JsonSerializer.Deserialize<RestDatabaseExecutableEntityVerboseSettings>((JsonElement)Rest, RuntimeConfig.SerializerOptions);
                    if (restSpSettings is not null)
                    {
                        return restSpSettings.RestMethods;
                    }
    ...
    /// <summary>
    /// Defines some utility extension functions for the SourceType enum
    /// </summary>
    ...
    /// <summary>
    /// Describes the REST settings specific to an entity backed by a stored procedure or a function.
    /// </summary>
    /// <param name="RestMethods">Defines the HTTP actions that are supported for stored procedures/functions.</param>
    public record RestDatabaseExecutableEntitySettings([property: JsonPropertyName("methods")] RestMethod[]? RestMethods = null);

    /// <summary>
    /// Describes the verbose REST settings specific to an entity backed by a stored procedure or a function.
    ...
    /// </param>
    /// <param name="RestMethods">Defines the HTTP actions that are supported for stored procedures/functions.</param>
    public record RestDatabaseExecutableEntityVerboseSettings(object? Path,
    ...
    /// <summary>
    /// Describes the GraphQL settings applicable to an entity which is backed by a stored procedure or a function.
    /// The GraphQL Operation denotes the field type generated for the stored procedure/function: mutation or query.
    /// </summary>
    /// <param name="GraphQLOperation">Defines the graphQL operation (mutation/query) that is supported for
    /// stored procedures/functions that will be used for this entity."</param>
    public record GraphQLDatabaseExecutableEntityOperationSettings([property: JsonPropertyName("operation")] string? GraphQLOperation = null);
    ...
    /// <param name="GraphQLOperation">Defines the graphQL operation (mutation/query) that is supported for
    /// stored procedures/functions that will be used for this entity."</param>
    public record GraphQLDatabaseExecutableEntityVerboseSettings([property: JsonPropertyName("type")] object? Type = null,
