        ...
        public string Build(SqlUpsertQueryStructure structure)
        {
            ...
            if (!structure.IsFallbackToUpdate)
            {
                ...
                if (!isInsertTriggerEnabled)
                {
                    if (isUpdateTriggerEnabled)
                    {
                        // This is just an optimisation.If update trigger is not enabled, then this build method has already created
                        // the appropriate columnsToBeReturned where each column has the Inserted prefix.
