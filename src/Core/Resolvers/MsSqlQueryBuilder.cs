...
namespace Azure.DataApiBuilder.Core.Resolvers
{
    ...
    public class MsSqlQueryBuilder : BaseSqlQueryBuilder, IQueryBuilder
    {
        ...
        public string Build(SqlInsertStructure structure)
        {
            ...
            List<string> allowedExposedColumns = structure.AuthorizationResolver.GetAllowedExposedColumns(structure.EntityName, "anonymous" , EntityActionOperation.Read).ToList();
            string columns = Build(allowedExposedColumns);
            
            string? policy = structure.AuthorizationResolver.ProcessDBPolicy(structure.EntityName, "anonymous", EntityActionOperation.Read, structure.HttpRequestContext!);
            
            FilterClause? filterClause = AuthorizationPolicyHelpers.GetDBPolicyClauseForQueryStructure(
                    policy,
                    entityName: structure.EntityName,
                    resourcePath: structure.DatabaseObject.FullName,
                    structure.MetadataProvider);

           string? policyQuery =  structure.GetFilterPredicates(filterClause, EntityActionOperation.Read);
           
            StringBuilder subsequentSelect = new($"SELECT {columns} FROM {tableName}");
            ...
            subsequentSelect.Append($" AND {policyQuery}");
