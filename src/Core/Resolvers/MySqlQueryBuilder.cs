...
namespace Azure.DataApiBuilder.Core.Resolvers
{
    ...
    public class MySqlQueryBuilder : BaseSqlQueryBuilder, IQueryBuilder
    {
        ...
        {
            (string sets, string updates, string select) = MakeStoreUpdatePK(structure, structure.OutputColumns);
            string predicates = JoinPredicateStrings(
...
WHERE
    ...
    REFERENCED_TABLE_NAME IN (@{tableNameParamsForInClause}))";
        ...
        /// </summary>
        private (string, string, string) MakeStoreUpdatePK(BaseSqlQueryStructure structure, List<LabelledColumn> outputColumns)
        {
            ...
            string sets = String.Join(";\n", columns.Where(col => !sourceDefinition.Columns[col].IsReadOnly).
                Select((x, index) => $"SET {"@LU_" + index.ToString()} := 0"));

            // Fetch the values of the updatable columns to local variables.
            string updates = String.Join(", ", columns.Where(col => !sourceDefinition.Columns[col].IsReadOnly).Select((x, index) =>
                $"{QuoteIdentifier(x)} = (SELECT {"@LU_" + index.ToString()} := {QuoteIdentifier(x)})"));

            // Select local variables and mapping to original column name.
            string select = String.Join(", ", outputColumns.Where(col => !sourceDefinition.Columns[col.ColumnName].IsReadOnly).
                Select((x, index) => $"{"@LU_" + index.ToString()} AS {QuoteIdentifier(x.Label)}"));
