...
namespace Azure.DataApiBuilder.Core.Services
{
    ...
    public class GraphQLSchemaCreator
    {
        ...
        private DocumentNode GenerateSqlGraphQLObjects(RuntimeEntities entities, Dictionary<string, InputObjectTypeDefinitionNode> inputObjects)
        {
            ...
            // Set of (source,target) entities with *:N relationship.
            // Keeping this hashset allows us to keep track of entities having *:N relationship without needing to traverse all the entities again.
            // This is required to compute the pair of entities having N:N relationship.
            HashSet<Tuple<string, string>> relationshipsWithRightCardinalityMany = new();

            // Set of (source,target) entities with N:N relationship.
            // This is evaluated with the help of relationshipsWithRightCardinalityMany.
            ...
            // Stores the entities which are not exposed in the runtime config but are generated by DAB as linking entities
            // required to generate object definitions to support nested mutation on entities with N:N relationship.
            List<string> linkingEntityNames = new();
            ...
            foreach ((string entityName, Entity entity) in entities)
            {
                ...
                if (!entity.GraphQL.Enabled)
                {
                    if (entity.IsLinkingEntity && sqlMetadataProvider.GetDatabaseType() is DatabaseType.MSSQL)
                    {
                        ...
                        linkingEntityNames.Add(entityName);
                    }
                ...
                if (sqlMetadataProvider.GetEntityNamesAndDbObjects().TryGetValue(entityName, out DatabaseObject? databaseObject))
                {
                    ...
                    if (rolesAllowedForEntity.Any())
                    {
                        ObjectTypeDefinitionNode node = SchemaConverter.FromDatabaseObject(
                            ...
                            relationshipsWithRightCardinalityMany,
        ...
        /// <param name="manyToManyRelationships">Collection of pair of entities with N:N relationship between them.</param>
        private void GenerateObjectDefinitionsForDirectionalLinkingEntities(
            ...
            HashSet<Tuple<string, string>> manyToManyRelationships)
        {
        ...
        private Dictionary<string, ObjectTypeDefinitionNode> GenerateObjectDefinitionsForLinkingEntities(List<string> linkingEntityNames, RuntimeEntities entities)
        {
            Dictionary<string, ObjectTypeDefinitionNode> linkingObjectTypes = new();
            foreach (string linkingEntityName in linkingEntityNames)
            {
                Entity linkingEntity = entities[linkingEntityName];
                string dataSourceName = _runtimeConfigProvider.GetConfig().GetDataSourceNameFromEntityName(linkingEntityName);
                ISqlMetadataProvider sqlMetadataProvider = _metadataProviderFactory.GetMetadataProvider(dataSourceName);
