...
namespace Azure.DataApiBuilder.Core.Telemetry.Product
{
    ...
    /// </summary>
    internal interface IProductTelemetryExporter<in T> : IDisposable where T : class, IProductTelemetryEvent
    {
        ValueTask<bool> ExportAsync(T record, CancellationToken cancellationToken);
    }
}
