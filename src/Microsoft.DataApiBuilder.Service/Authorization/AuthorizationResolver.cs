...
namespace Microsoft.DataApiBuilder.Service.Authorization
{
    /// <summary>
    ...
    public class AuthorizationResolver : IAuthorizationResolver
    {
            ...
            foreach ((string entityName, Entity entity) in runtimeConfig!.Entities)
            {
                ...
                foreach (PermissionSetting permission in entity.Permissions)
                {
                    ...
                    foreach (JsonElement operationElement in Operations)
                    {
                        ...
                        IEnumerable<Operation> operations = GetAllOperationsForObjectType(operation, entity.ObjectType);
        ...
        public static IEnumerable<Operation> GetAllOperationsForObjectType(Operation operation, SourceType sourceType)
        {
            if (sourceType is SourceType.StoredProcedure)
            {
                return new List<Operation> { Operation.Execute };
            }
        ...
        public IEnumerable<string> GetRolesForField(string entityName, string field, Operation operation)
        {
            ...
            // any role.
            if (EntityPermissionsMap[entityName].FieldToRolesMap.TryGetValue(field, out Dictionary<Operation, List<string>>? operationToRoles)
        ...
        private static Dictionary<Operation, List<string>> CreateOperationToRoleMap(SourceType sourceType)
        {
            if (sourceType is SourceType.StoredProcedure)
            {
                return new Dictionary<Operation, List<string>>()
                {
                    { Operation.Execute, new List<string>()}
                };
            }
