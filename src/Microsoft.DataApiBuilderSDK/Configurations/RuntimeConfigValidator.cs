...
using Microsoft.DataApiBuilderSDK.AuthenticationHelpers;
...
namespace Microsoft.DataApiBuilderSDK.Configurations
{
    /// <summary>
    ...
    public class RuntimeConfigValidator : IConfigValidator
    {
        ...
        public void ValidatePermissionsInConfig(RuntimeConfig runtimeConfig)
        {
            ...
            {
                entity.TryPopulateSourceFields();
                HashSet<Operation> totalSupportedOperationsFromAllRoles = new();
        ...
        public static bool IsValidPermissionAction(Operation action, Entity entity, string entityName)
        {
            if (entity.ObjectType is SourceType.StoredProcedure)
            {
                if (action is not Operation.All && !PermissionOperation.ValidStoredProcedurePermissionOperations.Contains(action))
                {
                    throw new DataApiBuilderException(
            ...
            {
                if (action is Operation.Execute)
                {
                    throw new DataApiBuilderException(
                        message: $"Invalid operation for Entity: {entityName}. " +
