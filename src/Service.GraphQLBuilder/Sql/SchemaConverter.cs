...
namespace Azure.DataApiBuilder.Service.GraphQLBuilder.Sql
{
    ...
    {
        /// <summary>
        /// Generate a GraphQL object type from a SQL table/view/stored-procedure/function definition, combined with the runtime config entity information
        /// </summary>
            ...
            IDictionary<string, IEnumerable<string>> rolesAllowedForFields)
        {
            ...
            if (databaseObject.SourceType.IsDatabaseExecutableType() && ((DatabaseExecutableDefinition)sourceDefinition).Columns.Count == 0)
            {
                FieldDefinitionNode field = GetDefaultResultFieldForDatabaseExecutable(databaseObject.SourceType);
            ...
            foreach ((string columnName, ColumnDefinition column) in sourceDefinition.Columns)
            {
                ...
                if (databaseObject.SourceType.IsDatabaseExecutableType() && sourceDefinition.PrimaryKey.Contains(columnName))
                {
                ...
                if (databaseObject.SourceType.IsDatabaseExecutableType() && column.IsAutoGenerated)
                {
                    directives.Add(new DirectiveNode(AutoGeneratedDirectiveType.DirectiveName));
                }

                if (databaseObject.SourceType.IsDatabaseExecutableType() && column.DefaultValue is not null)
                {
                ...
                if (rolesAllowedForFields.TryGetValue(key: columnName, out IEnumerable<string>? roles))
                {
                    ...
                    // Since Stored-procedures and Functions only support 1 CRUD action, it's possible that stored-procedures might
                    // return some values during mutation operation (i.e, containing one of create/update/delete permission).
                    // Hence, this check is bypassed for stored-procedures and functions.
                    if (roles.Count() > 0 || databaseObject.SourceType.IsDatabaseExecutableType())
                    {
