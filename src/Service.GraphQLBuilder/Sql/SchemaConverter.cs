...
namespace Azure.DataApiBuilder.Service.GraphQLBuilder.Sql
{
    public static class SchemaConverter
    {
            ...
            IDictionary<string, IEnumerable<string>> rolesAllowedForFields)
        {
            ...
            if (databaseObject.SourceType is EntityType.StoredProcedure && ((StoredProcedureDefinition)sourceDefinition).Columns.Count == 0)
            {
            ...
            foreach ((string columnName, ColumnDefinition column) in sourceDefinition.Columns)
            {
                ...
                if (databaseObject.SourceType is not EntityType.StoredProcedure && sourceDefinition.PrimaryKey.Contains(columnName))
                {
                ...
                if (databaseObject.SourceType is not EntityType.StoredProcedure && column.IsAutoGenerated)
                {
                    directives.Add(new DirectiveNode(AutoGeneratedDirectiveType.DirectiveName));
                }

                if (databaseObject.SourceType is not EntityType.StoredProcedure && column.DefaultValue is not null)
                {
                ...
                if (rolesAllowedForFields.TryGetValue(key: columnName, out IEnumerable<string>? roles))
                {
                    ...
                    if (roles.Count() > 0 || databaseObject.SourceType is EntityType.StoredProcedure)
                    {
