...
namespace Azure.DataApiBuilder.Service.GraphQLBuilder.Sql
{
    public static class SchemaConverter
    {
            ...
            IDictionary<string, IEnumerable<string>> rolesAllowedForFields)
        {
            ...
            if (configEntity.SemanticSearch?.Enabled is true)
            {
                List<DirectiveNode> semanticDistanceDirectives = [new DirectiveNode(AutoGeneratedDirectiveType.DirectiveName)];
                FieldDefinitionNode semanticDistanceField = new(
                    location: null,
                    name: new NameNode(QueryBuilder.SEMANTIC_DISTANCE_FIELD_NAME),
                    description: new StringValueNode(configEntity.SemanticSearch.OutputDescription),
                    arguments: [],
                    type: new NamedTypeNode(new NameNode(FLOAT_TYPE)),
                    directives: semanticDistanceDirectives);

                fieldDefinitionNodes.TryAdd(QueryBuilder.SEMANTIC_DISTANCE_FIELD_NAME, semanticDistanceField);
            }
