...
namespace Azure.DataApiBuilder.Service.Tests.Configuration
{
    ...
    : VerifyBase
    {
        ...
        /// <summary>
        /// Helper method to parse the config json string and set the connection string field to a given string.
        /// </summary>
        /// <param name="configJsonString">Config Json string</param>
        /// <param name="newConnectionString">Value to update the connection string with</param>
        /// <returns>Config Json string with connection string set to the given value</returns>
        private static string UpdateConnectionStringWithCustomValue(string configJsonString, string newConnectionString)
        {
            JObject configJson = JObject.Parse(configJsonString);
            configJson["data-source"]["connection-string"] = newConnectionString;
        ...
        {
            string referenceConfigJson = File.ReadAllText($"{CONFIGFILE_NAME}.{MSSQL_ENVIRONMENT}.{REFERENCE_CONFIG_FILE_SUFFIX}{CONFIG_EXTENSION}");
            string cliGeneratedConfigJson = File.ReadAllText($"{CONFIGFILE_NAME}.{MSSQL_ENVIRONMENT}{CONFIG_EXTENSION}");

            // Connection strings are ignored while comparing these two strings by setting the connection strings to an empty string.
            // This is necessary because the config json generated by CLI will contain the actual connection string to connect to the test database used by integration tests
            // whereas the reference config json will contain a sample value.
            SqlTestHelper.PerformTestEqualJsonStrings(UpdateConnectionStringWithCustomValue(referenceConfigJson, string.Empty), UpdateConnectionStringWithCustomValue(cliGeneratedConfigJson, string.Empty));
        }
        ...
        {
            string referenceConfigJson = File.ReadAllText($"{CONFIGFILE_NAME}.{MYSQL_ENVIRONMENT}.{REFERENCE_CONFIG_FILE_SUFFIX}{CONFIG_EXTENSION}");
        ...
        {
            string referenceConfigJson = File.ReadAllText($"{CONFIGFILE_NAME}.{POSTGRESQL_ENVIRONMENT}.{REFERENCE_CONFIG_FILE_SUFFIX}{CONFIG_EXTENSION}");
        ...
        {
            string referenceConfigJson = File.ReadAllText($"{CONFIGFILE_NAME}.{COSMOS_ENVIRONMENT}.{REFERENCE_CONFIG_FILE_SUFFIX}{CONFIG_EXTENSION}");
