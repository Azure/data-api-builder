...
namespace Azure.DataApiBuilder.Service.Tests.GraphQLBuilder
{
    ...
    public class GraphQLSchemaBuilderTests
    {
        ...
        public async Task CosmosSchemaBuilderTestAsync()
        {
            ...
            Assert.AreEqual(11, inputTypeObjects.Count, $"{nameof(InputObjectTypeDefinitionNode)} is invalid. input Type objects have not been created correctly.");

            // 11 input types generated for the 3 entity types in the schema.gql. IntFilter,StringFilter etc should not be duplicated.
            Assert.AreEqual(15, root.Definitions.Count, $"{nameof(DocumentNode)}:Root is invalid. root definitions count does not match expected count.");
        }
    }
}
