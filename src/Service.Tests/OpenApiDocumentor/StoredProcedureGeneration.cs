...
namespace Azure.DataApiBuilder.Service.Tests.OpenApiIntegration
{
    ...
    public class StoredProcedureGeneration
    {
        ...
        {
            Dictionary<HttpMethod, bool> configuredOperations = ResolveConfiguredOperations(_runtimeEntities[entityName]);
            foreach (HttpMethod opType in configuredOperations.Keys)
            {
                ...
                if(opType== HttpMethod.Get || opType == HttpMethod.Delete)
                {
                    continue;
                }

                OpenApiRequestBody requestBody = GetOperationRequestBody(entityName, opType);
                OpenApiSchemaReference schemaComponentReference = GetRequestBodyReference(requestBody);
        ...
        public void ValidateResponseBodyContents(string entityName, string[] expectedColumns, string[] expectedColumnJsonTypes)
        {
            ...
            OpenApiResponses responses = GetOperationResponses(entityName, HttpMethod.Get);
            ...
            OpenApiSchemaReference schemaComponentReference = GetResponseBodyReference((OpenApiResponse)responses[HttpStatusCode.OK.ToString("D")]);
        ...
        public void OpenApiDocumentor_TagsIncludeEntityDescription()
        {
            ...
            ISet<OpenApiTag> tags = _openApiDocument.Tags;
            ...
            string[] expectedPropertyJsonTypes)
        {
            ...
            Assert.AreEqual(expected: $"{SCHEMA_REF_PREFIX}{expectedSchemaReferenceId}", actual: $"#/components/schemas/{reference.Id}");
            ...
            var schemaComponentProperties = GetSchemaComponentProperties(expectedSchemaReferenceId);
            ...
            for (int propertyIdx = 0; propertyIdx < expectedProperties.Length; propertyIdx++)
            {
                ...
                var actualType = ((OpenApiSchema)schemaComponentProperties[propertyName]).Type;
                Assert.AreEqual(propertyType, actualType.ToString(), message: "Unexpected property JSON type in result schema component.");
            }
        }
        ...
        /// <param name="operationType">HTTP method type.</param>
        /// <returns></returns>
        public static OpenApiRequestBody GetOperationRequestBody(string entityName, HttpMethod operationType)
        {
            return (OpenApiRequestBody)_openApiDocument.Paths["/" + entityName].Operations[operationType].RequestBody;
        }
        ...
        public static OpenApiResponses GetOperationResponses(string entityName, HttpMethod operationType)
        {
        ...
        public static IDictionary<string, IOpenApiSchema> GetSchemaComponentProperties(string schemaComponentReferenceId)
        {
        ...
        public static OpenApiSchemaReference GetResponseBodyReference(OpenApiResponse response)
        {
            var items = ((OpenApiSchema)response.Content[CONTENT_TYPE].Schema).Properties[SCHEMA_PROPERTY_ACCESSOR].Items;
            return (OpenApiSchemaReference)items;
        }
        ...
        /// <returns>OpenApiReference containing the id of the schema component.</returns>
        public static OpenApiSchemaReference GetRequestBodyReference(OpenApiRequestBody requestBody)
        {
            return (OpenApiSchemaReference)requestBody.Content[CONTENT_TYPE].Schema;
        }
        ...
        /// <returns>Collection of HTTP methods and whether they should be created.</returns>
        public static Dictionary<HttpMethod, bool> ResolveConfiguredOperations(Entity entity)
        {
            ...
            foreach (SupportedHttpVerb restMethod in spRestMethods)
            {
                switch (restMethod)
                {
                    ...
                    case SupportedHttpVerb.Patch:
                        configuredOperations[HttpMethod.Patch] = true;
                        break;
