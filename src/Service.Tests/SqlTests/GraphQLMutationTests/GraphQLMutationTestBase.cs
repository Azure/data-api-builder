...
namespace Azure.DataApiBuilder.Service.Tests.SqlTests.GraphQLMutationTests
{
    ...
    public abstract class GraphQLMutationTestBase : SqlTestBase
    {
        ...
        {
            // Validate that insert operation succeeds when an insert DML trigger is enabled for a table with non-autogenerated primary key.
            // For non-autogenerated PK fields, the values are inserted into a temporary table and then an INNER JOIN is performed
            // based on those fields' values from the actual table and the temporary table.
            // Trigger behavior: The trigger updates the salary to max(0,min(30,salary)).
            // We would not need to specify the value of the PK column 'months' as it has a default value of 2.
        ...
        public async Task InsertMutationOnTableWithTriggerWithAutoGenPK(string dbQuery)
        {
            ...
            // with autogenerated primary key. For table containing an autogenerated PK field (id), and non-autogenerated PK field (u_id),
            // the values of the non-autogenerated PK fields are inserted into a temporary table and then an INNER JOIN is performed
            // based on those fields' values from the actual table and the temporary table. For the autogenerated PK field, an additional
            // WHERE predicate is added to fetch the unique record by extracting the value of the autogenerated PK field using the
            // SCOPE_IDENTITY() method provided by Sql Server.
            // Trigger behavior: The trigger updates the salary to max(0,min(100,salary)).
