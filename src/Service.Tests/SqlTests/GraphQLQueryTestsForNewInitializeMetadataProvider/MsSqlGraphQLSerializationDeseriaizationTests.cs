...
namespace Azure.DataApiBuilder.Service.Tests.SqlTests.GraphQLQueryTestsForNewInitializeMetadataProvider
{
    ...
    public class MsSqlGraphQLSerializationDeseriaizationTests : GraphQLQueryTestBase
    {
        /// <summary>
        ...
        public static async Task SetupAsync(TestContext context)
        {
            ...
            string entityDatabaseObjectJson = JsonSerializer.Serialize(_sqlMetadataProvider.EntityToDatabaseObject, options);
            string graphQLStoredProcedureExposedNameToEntityNameMapJson = JsonSerializer.Serialize(_sqlMetadataProvider.GraphQLStoredProcedureExposedNameToEntityNameMap, options);

            Dictionary<string, DatabaseObject> deserializedEntitiyToDatabaseObjectDictionary = JsonSerializer.Deserialize<Dictionary<string, DatabaseObject>>(entityDatabaseObjectJson, options)!;
            Dictionary<string, string> deserializedGraphqlStoredProcedureDictionary = JsonSerializer.Deserialize<Dictionary<string, string>>(graphQLStoredProcedureExposedNameToEntityNameMapJson, options)!;

            _sqlMetadataProvider.EntityToDatabaseObject = new();
            _sqlMetadataProvider.GraphQLStoredProcedureExposedNameToEntityNameMap = new();

            _sqlMetadataProvider.InitializeAsync(deserializedEntitiyToDatabaseObjectDictionary, deserializedGraphqlStoredProcedureDictionary);
        }
