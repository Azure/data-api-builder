...
namespace Azure.DataApiBuilder.Service.Tests.SqlTests.RestApiTests.Put
{
    ...
    public abstract class PutApiTestBase : RestApiTestBase
    {
        ...
        /// <summary>
        /// Test to validate that PUT operation which satisfies either or both the database policies for update/create operation
        ...
        {
            // Validate that a PUT operation on table with autogenerated primary key (resolves to update),
            // which satisfies the database policy ("@item.id ne 1234"), executes successfully.
            string requestBody = @"
            {
                ""name"": ""New Publisher""
            }";
            ...
            // Validate that PUT operation satisfying the database policy for update operation ("@item.categoryid ne 0")
            // and having a record corresponding to provided primary key successfully updates the record.
            requestBody = @"
            {
                ""categoryName"": ""SciFi"",
                ""piecesRequired"": 5,
            ...
            await SetupAndRunRestApiTest(
                    primaryKeyRoute: "categoryid/100/pieceid/0",
                    queryString: null,
                    entityNameOrPath: _Composite_NonAutoGenPK_EntityPath,
                    sqlQuery: GetQuery("PutOneUpdateWithUpdateDatabasePolicySatisfied"),
                    operationType: Config.Operation.Upsert,
                    requestBody: requestBody,
            ...
            // Validate that PUT operation on a table which does not satisfy database policy for update operation ("@item.categoryid ne 0")
            // but satisfies the database policy for create operation ("@item.pieceid ne 6"), inserts a record successfully.
            requestBody = @"
            {
                ""categoryName"": ""SciFi""
            }";

            string expectedLocationHeader = "categoryid/0/pieceid/5";
            await SetupAndRunRestApiTest(
                    primaryKeyRoute: expectedLocationHeader,
                    queryString: null,
                    entityNameOrPath: _Composite_NonAutoGenPK_EntityPath,
                    sqlQuery: GetQuery("PutOneInsertWithInsertDatabasePolicySatisfied"),
                    operationType: Config.Operation.Upsert,
                    requestBody: requestBody,
                    clientRoleHeader: "database_policy_tester",
            ...
            // Validate that when a PUT operation satisfies the database policies for both update operation ("@item.categoryid ne 0")
            // and create operation ("@item.pieceid ne 6") but no record corresponding to given primary key exists, a new record is created successfully.
            requestBody = @"
            {
                ""categoryName"": ""SciFi""
            }";

            expectedLocationHeader = "categoryid/1/pieceid/5";
            await SetupAndRunRestApiTest(
                    primaryKeyRoute: expectedLocationHeader,
                    queryString: null,
                    entityNameOrPath: _Composite_NonAutoGenPK_EntityPath,
                    sqlQuery: GetQuery("PutOneInsertWithBothDatabasePoliciesSatisfied"),
                    operationType: Config.Operation.Upsert,
                    requestBody: requestBody,
                    clientRoleHeader: "database_policy_tester",
                    expectedStatusCode: HttpStatusCode.Created,
