...
namespace Azure.DataApiBuilder.Service.Tests.UnitTests
{
    ...
    public class SqlQueryExecutorUnitTests
    {
        ...
        /// </summary>
        [TestMethod, TestCategory(TestCategory.MSSQL)]
        public async Task TestCancellationTokenTimeoutDuringQueryExecution()
        {
            RuntimeConfig mockConfig = new(
               Schema: "",
               DataSource: new(DatabaseType.MSSQL, "", new()),
        ...
        /// <summary>
        /// Validates that when ExecuteQueryAgainstDbAsync throws TaskCanceledException
        /// (a subclass of OperationCanceledException, commonly thrown when an HTTP request
        /// is aborted), the retry policy does NOT retry the operation.
        /// </summary>
        [TestMethod, TestCategory(TestCategory.MSSQL)]
        public async Task TestTaskCancelledExceptionIsNotRetriedByRetryPolicy()
        {
            RuntimeConfig mockConfig = new(
               Schema: "",
               DataSource: new(DatabaseType.MSSQL, "", new()),
            ...
            // Mock ExecuteQueryAgainstDbAsync to throw TaskCanceledException,
            // simulating an HTTP request timeout or client disconnect.
            queryExecutor.Setup(x => x.ExecuteQueryAgainstDbAsync(
                It.IsAny<SqlConnection>(),
                It.IsAny<string>(),
                It.IsAny<IDictionary<string, DbConnectionParam>>(),
