    ...
    public class AuthorizationResolver : IAuthorizationResolver
    {
        ...
        public void SetEntityPermissionMap(RuntimeConfig? runtimeConfig)
        {
            foreach ((string entityName, Entity entity) in runtimeConfig!.Entities)
            {
                ...
                bool isDatabaseExecutableEntity = entity.ObjectType.IsDatabaseExecutableType();
                if (isDatabaseExecutableEntity)
                {
                    RestMethod[]? methods = entity.GetRestMethodsConfiguredForDatabaseExecutable();
                    if (methods is not null)
                    {
                        entityToRoleMap.DatabaseExecutableHttpVerbs = new(methods);
                    }
                }
        ...
        /// Stored procedures and Functions only support Operation.Execute.
        /// In case the operation is Operation.All (wildcard), it gets resolved to a set of CRUD operations.
        /// </summary>
        /// <param name="operation">operation type.</param>
        /// <param name="sourceType">Type of database object: Table, View, Stored Procedure, or Function.</param>
