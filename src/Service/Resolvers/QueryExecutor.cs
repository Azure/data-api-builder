        ...
        where TConnection : DbConnection, new()
    {
            ...
            DbDataReader dbDataReader, List<string>? args)
        {
            ...
            else if (await dbDataReader.NextResultAsync())
            {
                ...
                {
                    // Assuming the update operation returned zero rows because we didn't find a record with given primary key, the insert
                    // operation should have succeeded and returned the created record or thrown an exception.
                    // However, we don't get any exception and yet no record is returned.
                    // This indicates that the database policy for both update and insert operations were not satisfied.
            ...
            {
                // This is the case where UPDATE returned no records for a table/view with autogenerated primary key.
                // e.g. a situation where the item with the given PK doesn't exist or the database policy for update operation is not satisfied,
                // so there's no update and since PK is auto generated, so insert can not happen.
