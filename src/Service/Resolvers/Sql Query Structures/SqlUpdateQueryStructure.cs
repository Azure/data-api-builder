        ...
        private Predicate CreatePredicateForParam(KeyValuePair<string, object?> param)
        {
            ...
            else if (param.Value is null)
            {
                predicate = new(
                    ...
                    new PredicateOperand($"{MakeParamWithValue(null, backingColumn)}")
                );
            }
            else
            {
                predicate = new(
                    ...
                    PredicateOperation.Equal,
                    new PredicateOperand($"{MakeParamWithValue(GetParamAsSystemType(param.Value.ToString()!, param.Key, GetColumnSystemType(param.Key)), param.Key)}"));
            }
