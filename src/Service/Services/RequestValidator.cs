    ...
    public class RequestValidator
    {
            ...
            ISqlMetadataProvider sqlMetadataProvider)
        {
            ...
            foreach ((string colName, ColumnDefinition colDef) in baseTableDefinition.Columns)
            {
                ...
                {
                    // If the column in the base table requires a value to be specified,
                    // we cannot perform insert on the base table for this request.
                    if (!colDef.IsNullable && !colDef.HasDefault && !colDef.IsAutoGenerated)
                    {
                        throw new DataApiBuilderException(
                            message: $"Cannot perform this operation on the underlying base table.",
                            statusCode: HttpStatusCode.BadRequest,
                            subStatusCode: DataApiBuilderException.SubStatusCodes.BadRequest);
                    }
            ...
            ISqlMetadataProvider sqlMetadataProvider)
        {
            ...
            foreach ((string colName, ColumnDefinition colDef) in baseTableDefinition.Columns)
            {
                ...
                if (!sqlMetadataProvider.TryGetExposedColumnName(
                    ...
                    out string? exposedName))
                {
                    // In case of PUT operation, the request would fail if the column missing
                    // in view is not nullable, doesn't have default value and is not autogenerated.
                    // For rest of the cases, we can't say for sure.
                    if (upsertRequestCtx.OperationType is Operation.Upsert &&
                        !colDef.IsNullable && !colDef.HasDefault && !colDef.IsAutoGenerated)
                    {
                        throw new DataApiBuilderException(
                            message: $"Cannot perform this operation on the underlying base table.",
                            statusCode: HttpStatusCode.BadRequest,
        ...
        {
            SourceType sourceTypeOfEntity = requestCtx.DatabaseObject.SourceType;
